...
namespace EquipmentSystem.Tests
{
    public abstract class EquipmentSystemTestBase : AbpIntegratedTestBase<EquipmentSystemTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected EquipmentSystemTestBase()
        {
        ...
        protected void UsingDbContext(Action<EquipmentSystemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EquipmentSystemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EquipmentSystemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EquipmentSystemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EquipmentSystemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EquipmentSystemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EquipmentSystemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EquipmentSystemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EquipmentSystemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EquipmentSystemDbContext, Task<T>> func)
        {
            T result;
